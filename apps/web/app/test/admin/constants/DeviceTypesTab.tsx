...
const API_ENDPOINT = '/admin/api/v1/device-types';

export default function DeviceTypesTab({ token }: DeviceTypesTabProps) {
  const [deviceTypes, setDeviceTypes] = useState<DeviceType[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [editingDeviceType, setEditingDeviceType] = useState<Partial<DeviceType> | null>(null);
  ...
  const handleOpenEditModal = (deviceType: DeviceType) => {
  ...
  const handleSave = async (data: Partial<DeviceType>) => {
    if (!token) return;
  ...
  return (
    <div>
        ...
        <tbody className="divide-y divide-gray-200 dark:divide-gray-800 bg-white dark:bg-gray-900">
          {deviceTypes.map((item) => (
            <tr key={item.id}>
              ...
              <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-600 dark:text-gray-300">{item.code}</td>
              <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-600 dark:text-gray-300">{item.description}</td>
              <td className="whitespace-nowrap px-3 py-4 text-sm text-gray-600 dark:text-gray-300">{item.sort_key}</td>
      ...
      <Modal 
        ...
        title={editingDeviceType?.id ? '기기 타입 수정' : '기기 타입 생성'}
      >
        <DeviceTypeForm 
